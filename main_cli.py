...
def main():
    ...
    # Cluster pockets
    parser_convert = subparsers.add_parser("cluster_pockets",help="Clusters identified pockets based on binary feature vectors "
                                           "denoting residue presence in the pocket.")
    parser_convert.add_argument("--infolder", type=str, help='Input folder containing PDB files (should be generated by extract_to_pdb).')
    parser_convert.add_argument("--outfolder", type=str, help='Output folder.')
    parser_convert.add_argument("--method", type=str, choices=["hierarchical","dbscan"], 
                                help='Clustering method. Choose between hierarchical and dbscan.')
    parser_convert.add_argument("--depth", type=int, default=4, 
